...
// header = title bar, body = background. Applied to groups as: group.color = header.
const THEMES = [
    // ── Muted / deep ─────────────────────────────────────────────────────────
    { name: "Ocean",    header: "#1e4d73", body: "#0d2035" },
    { name: "Forest",   header: "#1e5234", body: "#0d2418" },
    { name: "Dusk",     header: "#421e6b", body: "#200d33" },
    { name: "Ember",    header: "#6b300e", body: "#301607" },
    { name: "Rosewood", header: "#6b1e38", body: "#300d1a" },
    { name: "Slate",    header: "#2c404f", body: "#141d23" },
    { name: "Sage",     header: "#2c5040", body: "#14231c" },
    { name: "Amber",    header: "#5c3d0e", body: "#2a1c07" },
    { name: "Midnight", header: "#1c1c3e", body: "#0d0d1e" },
    { name: "Mauve",    header: "#4c2840", body: "#23121e" },
    { name: "Storm",    header: "#2c3d52", body: "#131a22" },
    { name: "Rust",     header: "#5c2a1c", body: "#2a120d" },
    // ── Vibrant ───────────────────────────────────────────────────────────────
    { name: "Cobalt",   header: "#1a5fb5", body: "#0c2a52" },
    { name: "Jade",     header: "#107360", body: "#063028" },
    { name: "Violet",   header: "#6d21b8", body: "#2f0e52" },
    { name: "Crimson",  header: "#991520", body: "#40090e" },
    { name: "Teal",     header: "#0d7a8a", body: "#05363e" },
    { name: "Coral",    header: "#c03d12", body: "#521a08" },
    { name: "Plum",     header: "#7e1a90", body: "#360a3e" },
    { name: "Royal",    header: "#1e44b8", body: "#0d1e52" },
    { name: "Pine",     header: "#16693c", body: "#0a2e1a" },
    { name: "Indigo",   header: "#3a2ea8", body: "#181450" },
    { name: "Garnet",   header: "#8b1a2f", body: "#3d0b15" },
    { name: "Copper",   header: "#a04a14", body: "#461f08" },
];
...
function showThemeDialog(nodes, group) {
    openDialog(dialog => {
        ...
        titleEl.style.cssText = "font-weight:600;font-size:15px;color:#fff;margin-bottom:4px;";
        const parts = [];
        if (nodes.length > 0) parts.push(`${nodes.length} node${nodes.length > 1 ? "s" : ""}`);
        else if (group)        parts.push("1 group");
        const subEl = Object.assign(document.createElement("div"), {
            textContent: parts.length ? "Applying to: " + parts.join(", ") : "Select nodes or a group first",
        });
        subEl.style.cssText = "font-size:12px;color:#777;margin-bottom:16px;";
        dialog.append(titleEl, subEl);
        ...
        THEMES.forEach(theme => {
            ...
            preview.style.cssText = "width:22px;height:30px;border-radius:3px;overflow:hidden;border:1px solid #3a3a5a;flex-shrink:0;display:flex;flex-direction:column;";
            ...
            card.append(preview, nameEl);
            card.addEventListener("click", () => {
                // Apply to nodes if selected, otherwise to group — not both at once
                if (nodes.length > 0) {
                    nodes.forEach(n => { n.color = theme.header; n.bgcolor = theme.body; });
                } else if (group) {
                    group.color = theme.header;
                }
                closeDialog(); redraw();
            });
